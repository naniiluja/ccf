...
**Live iteration as of 2026-08-02: prompt-standard, tasks 045 to 048 — all four at `in-review`.** It rewrote all 13 plugin prompts onto `.claude/rules/prompt-standard.md` (checklist, canonical style block, codepoint policy, `FAIL:`/`WARN:`/`PASS:` markers). Live checks now observed: 045 (a full `/ccf:plan` run), 046 (a real `/ccf:check` pass), and — via the latch-hardening iteration below — `cook.md` itself. Still owed before any row goes `done`: a `/code-review` over their diff (they were committed straight to `main` with no PR; PRs #2/#3 cover only 049/050) or a user acceptance recorded by name, plus 047's narrow checks (`/ccf:fix` to the model question, `/ccf:init` to A4).

**Latch-hardening (049 + 050) is CLOSED and retired to `ARCHIVE.md` — the first iteration whose every gate was observed, not bulk-closed.** 049 (PR #2): the `FAIL:` marker locked into the `buildVerifyReason` tests, the four-times-drifted paid figure moved into the machine-checked `ccf-budget` label (`.claude/tests/context-budget.test.mjs` went RED for real twice while the label was stale — the latch bites), `cook.md` joined `check.md` on bare `Bash`. 050 (PR #3, executed by the FIRST live `/ccf:cook`): templates onto the prompt standard, `PLAN.md.tmpl` gained `## Origin: {{ITERATION_NAME}}` with the guidance blockquotes in the retirement-safe preamble, and the vacuous first version of that latch was caught and replaced with red-then-green-verified cases (a lesson now recorded in `.claude/rules/testing.md`). The four cook observations (backlog selection, `Model:` line, sequential `run_in_background: false`, bare `Bash` beyond the old npx/node/claude trio) live in `archive/task-050-templates-standard.md`.
