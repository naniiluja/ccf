...
10. **Any prompt step that writes code carries an explicit anti-over-engineering block.** All coding happens directly in a command now (`plan.md` step 6, `cook.md` step 2), so those are the only places where scope creep turns into committed code, and each names the same discipline: build only what the acceptance criteria require, no speculative abstraction, no drive-by refactor.
11. **Cut line by line, with a hard exemption list.** Every line must add information the model does not already have. Never cut: the prompt-side backup for a fail-open hook (`plan.md` step 0, `plan-mode-guard.mjs`'s only prompt-side backup), any string another file parses (`discipline: on`, the status words, the `FAIL:`/`WARN:`/`PASS:` markers), or a step number another file cites. When a rewrite deletes lines, list them.
...
Verify the copies with `grep -rln "^- Write in the SAME language" plugins/ccf`, then md5 each block against the hash above. Currently **7** files (`plan.md`, `init.md`, `check.md`, `cook.md`, `updatespec.md`, `ccf-spec-writer.md`, `ccf-spec-checker.md`) — down from 9 after `fix.md` and `ccf-debugger.md` were retired, since both had carried a copy.
...
**A fifth exemption, whole directories rather than one file, recorded here at task 050:** everything under `plugins/ccf/templates/` (`.tmpl` files AND a plain `.md` usage-hint file like `settings.json.tmpl.md`) never carries the block, for three reasons. First, a template is the mold for another PROJECT's `CLAUDE.md`/rules, and that project has its own language and its own conventions filled in by `/ccf:init` — the block's own bullets ("Write in the SAME language the user is using") make no sense pasted into a file nobody has written a conversation against yet. Second, this rule's `paths:` frontmatter scopes to `plugins/ccf/{commands,agents,skills}/**`, which does not cover `templates/**`, so a template edit never lazy-loads this file — pasting the block there would be an unenforceable copy nothing here ever checks. Third, the byte-identical copy count the "Verify the copies with `grep`" sentence pins above would drift the moment a template variant is added or removed, for a check that gains nothing since template prose is never read by the model in THIS repo's own session. Templates instead get their own lighter pass: affirmative instructions with reasons, no icons, and the word markers `FAIL:`/`WARN:`/`PASS:` if a template ever needs a review marker at all.
