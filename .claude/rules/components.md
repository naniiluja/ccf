...
- Frontmatter: `name` (kebab-case, matches the filename), `description`, `model`, `effort` (optional — reasoning depth, see below). Tool policy is **inheritance, NOT a `tools` allowlist** (grounded, `code.claude.com/docs/en/sub-agents` + `/en/tools-reference`):
  ...
  - **`background` frontmatter field**: NOT ignored for a plugin agent, but per changelog it is documented for `true` only (forces the agent to always run in the background) — a `false` value is undocumented, so do NOT set `background: false` expecting it to force synchronous execution. The sequential law is instead held at the CALL SITE via `run_in_background: false` (see `architecture.md`'s "Background-by-default" note and `tooling.md`).
  - **Environment ceilings**: per changelog, at most 20 subagents run concurrently, and at most 200 spawns total per session. CCF's own usage (≤5 parallel read-only research, 1 writer at a time) stays far below both, but a large `/ccf:cook` backlog run should keep this ceiling in mind.
  - **`model` and `effort` frontmatter are DEFAULTS only**, not a fixed choice: the spawning call site can override either, and when the USER expresses a preference (budget, urgency, task difficulty they alone can judge) that choice WINS over the frontmatter default. See `cook.md`'s per-task `Model:` line lookup for a concrete example.
  - **Tool-name duality reaches command permissions too, not only hook transcripts**: the 6 commands' `allowed-tools` declare the spawn tool as `Task` (matching the documented name), yet the transcript-observed spawn tool name is `Agent` in this harness (same finding already recorded in `hooks.md`'s `SPAWN_TOOL_NAMES`) — and spawning still succeeds under the `Task`-only allowlist. This is why agent frontmatter's `disallowedTools` deliberately lists BOTH `Agent, Task` rather than only the documented name — it must block whichever name the harness actually uses.
  - **Deliberately NOT added to any agent's frontmatter**: `maxTurns`, `skills`, `memory`, `isolation`. No real need has been demonstrated yet and none is grounded in the docs consulted so far — do not add speculatively (YAGNI).
