...
- `/cook` — **use when**: you want to run the WHOLE todo/in-progress backlog in one go instead of a fresh session per task — sequential `ccf-implementer` loop (stop on any red gate) then a batch-verify pass (review + `/code-review` in parallel, `/simplify`, re-gate, `/ccf:ccf-updatespec`). **Mutually exclusive with `auto-verify.mjs --auto-verify`** — `/cook` drives the same verify chain itself; don't enable both.
