# Task 032 — Lệnh `/ccf-cook` (orchestrator tuần tự → batch-verify)

> **SUPERSEDED PARTIALLY (post-hoc cleanup):** lệnh ban đầu tên `/cook` (`commands/cook.md`) → đã đổi tên `/ccf-cook` (`commands/ccf-cook.md`); command `/ccf-test` đã bị XÓA nên "count sync 6→7" dưới đây KHÔNG còn đúng — count net = **6 command**. Xem iteration "retire-ccf-test + rename-cook" ở đầu `PLAN.md`. Các tiêu chí count/`4rum.md` bên dưới là point-in-time.

- **Vertical slice:** file mới `commands/ccf-cook.md` + wire `ccf-plan` step 8 + entry `tooling.md` + count sync 6→7 command (một command cố kết end-to-end; nửa verify gánh rủi ro nhưng là 1 file `.md` nên fold hợp lệ).
- **Depends on:** 031 (chặn nested-spawn trước khi `/ccf-cook` spawn implementer hàng loạt).
...
- **Gate (must be GREEN before task 032a):** `claude plugin validate plugins/ccf` nhận `/ccf-cook` + grep-consistency (`7 command` trên doc SỐNG; grep `6 command`=0 hit CHỈ trên doc sống, LOẠI TRỪ `task-*.md` + PLAN.md "Closed"/narrative; `/ccf-cook` ở plan/tooling/README) + đọc lại `ccf-cook.md` (allowed-tools tối thiểu, không dựa SlashCommand, có mục quan hệ `auto-verify.mjs` + re-gate sau simplify) + `npx -p typescript tsc --noEmit` + `node --test` baseline.

## Goal (one sentence)
Tạo `/ccf-cook`: sau `/ccf-plan`, một lệnh chạy tuần tự toàn bộ backlog bằng `ccf-implementer` (Sonnet 5, gate xanh từng slice), rồi batch-verify (2 read-only song song → re-gate sau simplify → updatespec), loại-trừ với `auto-verify.mjs`.

## Acceptance criteria (verifiable)
- [ ] NEW `plugins/ccf/commands/ccf-cook.md`, frontmatter: `description`, `argument-hint: "[optional: task range]"`, `allowed-tools: Read, Glob, Grep, Task, Skill` (KHÔNG SlashCommand), `model: opus`.
...
- [ ] `ccf-cook.md` ghi rõ: "≤3 agent" chỉ áp cho CCF-tự-spawn (`ccf-spec-checker`); built-in `/code-review`/`/simplify` fan-out nội bộ chỉnh qua `effort`, không cap bằng số (`/simplify` cố định 4).
- [ ] `ccf-plan.md` step 8: thêm dòng gợi ý chạy `/ccf-cook` để thực thi cả backlog (đặt cạnh khuyến nghị fresh-session hiện có, KHÔNG xoá nó — `/ccf-cook` là lựa chọn tiện lợi, fresh-session-per-task vẫn là mặc định chất lượng).
- [ ] `.claude/rules/tooling.md` "CCF self-checks": thêm entry `/ccf-cook` (when-to-use — chạy cả backlog + batch-verify; loại-trừ `--auto-verify`).
...
1. `claude plugin validate plugins/ccf` — sau khi thêm `ccf-cook.md` vẫn exit 0 (nhận command mới).
...
3. `grep -rln "cook" plugins/ccf/commands/ .claude/rules/tooling.md` — sau: `ccf-cook.md` + tooling.md có mặt.

## Files to touch
- `plugins/ccf/commands/ccf-cook.md` — NEW
- `plugins/ccf/commands/ccf-plan.md` — step 8 thêm gợi ý `/ccf-cook`
- `.claude/rules/tooling.md` — entry `/ccf-cook`
...
3. Viết `ccf-cook.md` (8 bước, allowed-tools tối thiểu, không SlashCommand, mục auto-verify + re-gate).
