# Task 032a — Live post-reload verify của `/ccf-cook` (treo, HARD gate)

- **Vertical slice:** quan sát hành vi THẬT của `/ccf-cook` sau khi reload plugin — không viết code, chỉ verify + ghi bài học (independent green gate = driver right-size hợp lệ để tách khỏi 032).
- **Depends on:** 032.
...
## Goal (one sentence)
Xác nhận trên harness thật rằng `/ccf-cook` chạy đúng: implementer tuần tự bằng Sonnet 5 + KHÔNG nest-spawn, và chuỗi batch-verify chạy được (hoặc rơi fallback nhắc-tay) — ghi lại cơ chế invocation thật.

## Acceptance criteria (verifiable, observed)
- [ ] User reload plugin (`plugins/ccf` cache refresh) — xác nhận version/artefact-count mới (6 command sau retire-ccf-test) active.
- [ ] Chạy `/ccf-cook` trên 1 backlog NHỎ (1–2 task giả lập hoặc task thật nhỏ).
