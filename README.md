...
- **Hooks** = 7 `.mjs` run directly with `node` — no build step, no dependency, Windows-clean; shared helpers (freshness, plan parsing, context-usage, review-trace, git-trace, verify-trace, output-style, explore-guide) live in `hooks/lib/`.
