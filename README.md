...
| **auto-verify** | `Stop` | **Opt-in** (default off) and the only CCF Stop hook that can **block**. Enable by adding `--auto-verify` to the `auto-verify.mjs` command in `hooks.json`. When a task is **in-review**, this session **changed code**, and no `ccf-spec-checker` review has run yet, it returns `decision: "block"` (the "ralph loop") with a reason that drives the main loop through the verify chain — `/ccf:ccf-check` → `/code-review` → (`/ccf:ccf-test` if the test discipline is on) → `/ccf:ccf-updatespec` only when both come back clean. Guards against loops via `stop_hook_active`; best-effort, any error exits silently. |
