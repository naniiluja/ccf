...
- **Commands** = 5 markdown prompts that drive Claude in-session (not scripts): init, plan, check, updatespec, cook.
...
- **Hooks** = 6 `.mjs` run directly with `node` — no build step, no dependency, Windows-clean; shared helpers (freshness, plan parsing, context-usage, review-trace, git-trace, verify-trace, verify-chain, explore-guide, archive) live in `hooks/lib/`.
