...
| `/ccf:plan` | Tạo plan tuần tự cho một feature, grounded trong best practice. **Yêu cầu plan mode** (Shift+Tab) — được hook bắt buộc. Sau plan, execute từng task bằng agent. |
| `/ccf:check` | Verify implementation so với spec (conformance, convention, SOLID/OOP, cross-check BE↔FE). Read-only. |
| `/ccf:fix` | Debug có kỷ luật: tái hiện → trace log/DB từng bước → root cause → failing test → fix tối thiểu. Không đoán mò. |
| `/ccf:updatespec` | Cập nhật spec **và memory hệ thống** với bài học trong session (gồm công cụ mới kèm "dùng khi nào"). |
| `/ccf:cook` | Chạy toàn bộ backlog todo/in-progress trong một lần: vòng lặp tuần tự `ccf-implementer` (dừng ngay khi gate đỏ), rồi batch-verify (review + `/code-review` song song, `/simplify`, re-gate, `/ccf:updatespec`). Loại trừ lẫn nhau với `auto-verify.mjs --auto-verify`. |

Luồng điển hình: `/ccf:init` → (plan mode) `/ccf:plan` → implement (từng task, hoặc cả backlog qua `/ccf:cook`) → `/ccf:check` → `/code-review` → `/ccf:updatespec`. Khi test discipline bật, `ccf-implementer` viết ma trận test mức contract ngay trong lúc implement và chuỗi verify sẽ chạy chúng.

## 6 agent

Các subagent chuyên biệt **kế thừa tool, MCP server và skill của dự án host** — nên chúng dùng được bất kỳ MCP nào dự án bạn cung cấp (Supabase, Oracle, chrome-devtools, …) và gọi được skill của dự án, không phải bảo trì allowlist riêng cho từng agent. Mọi agent CCF đều là **leaf** — mang `disallowedTools: Agent, Task` nên không spawn được agent con (mặc định vẫn cho nest-spawn, nhưng giới hạn thay đổi theo bản: 5 ở bản v2.1.172 tới v2.1.216, 1 — coi như tắt hẳn — ở bản v2.1.217 và v2.1.218, rồi trở lại 3 từ bản v2.1.219 trở đi, đây là mặc định hiện hành; đổi được bằng biến môi trường `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` — đặt bằng `1` chỉ tắt spawn LỒNG NHAU, nghĩa là agent con không spawn được agent con của riêng nó nữa; harness vẫn spawn agent ở tầng 1 như bình thường. CCF vẫn chặn spawn lồng nhau một cách xác định bất kể con số mặc định của môi trường). Các agent read-only (tất cả trừ `ccf-implementer`) còn liệt kê thêm `Write, Edit, NotebookEdit`, nên cũng có cùng tầm với MCP/skill nhưng **không ghi file được**. Parallelism **chỉ dành cho research read-only** — agent ghi file không bao giờ chạy song song trên cùng một feature.
...
| **implementer-verify-gate** | `SubagentStop` (`ccf-implementer`) | **Opt-in** (mặc định tắt), phạm vi chỉ trong subagent con. Bật bằng cách thêm `--enforce-tests` vào lệnh `implementer-verify-gate.mjs` trong `hooks.json`. Khi tin nhắn cuối của `ccf-implementer` được spawn không có bằng chứng `TEST-RESULT:` (dòng Return-format đã ghim — kết quả thật HOẶC khai báo prose-only `TEST-RESULT: n/a (no test surface)`), nó trả về `decision: "block"` yêu cầu implementer thêm dòng đó rồi hoàn thành lại; điều này chỉ giữ SUBAGENT chạy tiếp, không ảnh hưởng main loop. Thiết kế phòng thủ: hình dạng loop-guard/`transcript_path` của SubagentStop chưa được quan sát trên harness thật tại thời điểm viết; best-effort, mọi lỗi đều thoát im lặng. **`stop_hook_active` chỉ là cơ chế "hỏi một lần", KHÔNG phải bảo đảm cưỡng chế**: nếu field này tồn tại và là `true` ở lần stop kế tiếp, lần stop đó vẫn được cho qua dù vẫn thiếu bằng chứng TEST-RESULT — nó chỉ hỏi lại một lần để tránh vòng lặp vô hạn. |
...
- **Command** = 6 file markdown prompt điều khiển Claude trong session (không phải script): init, plan, check, fix, updatespec, cook.
