...
4. **Write the failing test first** (per `testing.md`), run it to confirm it's red.
5. Implement the minimum to make the test green + meet the acceptance criteria.
6. Re-run the test, report actual results.
7. Update the task status in `.claude/plan/PLAN.md` (`in-progress` → `done`).
