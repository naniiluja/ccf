...
## 6. Review the plan (fresh-context, staff-engineer)
Before handing the plan to implementation, get a second pair of eyes on the PLAN ITSELF (not the code): delegate to a fresh-context `ccf-spec-checker` subagent (via Task, read-only) to critique it as a staff engineer would — are slices truly vertical/independent? gates real and verifiable? any task hiding multiple concerns or a missing predecessor? drift from the spec? Fold the critique back in before asking the user to approve. (This is read-only review, so it's an allowed use of a subagent under the sequential law.)
