...
## 8. Closing
Advise executing each task in a fresh session; after implementation, recommend `/ccf:ccf-check` → `/code-review` → `/ccf:ccf-updatespec`. If the project opted into the test discipline, also recommend `/ccf:ccf-test [function/slice]` to design the contract-level matrix and run it before `/code-review`. Fresh-session-per-task stays the quality default — but if the user wants to run the whole backlog in one go, point them at **`/cook`**: it drives the sequential implement loop + batch-verify (review + `/code-review` in parallel, `/simplify`, re-gate, `/ccf:ccf-updatespec`) automatically as a convenience option.
