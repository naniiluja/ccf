...
> If the bug has ≥2 independent hypothesis branches, you MUST hand each branch to a `ccf-debugger` subagent (read-only) via Task **with `run_in_background: false`** to isolate them in parallel without flooding context, and wait for every branch to report back before step 3 — since Claude Code v2.1.198 an omitted `run_in_background` defaults to background, which could let step 3 judge the bug before a branch has actually reported. This is the read-only-research EXCEPTION to the sequential law (parallel read-only research is allowed per `architecture.md`), so it does NOT violate "no parallel work". **Model — ASK with `AskUserQuestion`, once, before spawning.** **Recommend `opus`** and label it as the recommendation: root-cause work is the high-stakes reasoning the agent's frontmatter default is chosen for. Offer `sonnet` as the cheaper choice for a bug the user already considers simple, `haiku` only if they ask. Accept a model ALIAS only, never a dated model ID. Do NOT silently spawn the session's own model just because it is what the main loop runs as — the agent's `model` frontmatter is a DEFAULT, and overriding it at the call site without asking is exactly the behavior this forbids. If `AskUserQuestion` is genuinely unavailable (non-interactive), fall back to `opus` and say explicitly that the default was used because asking was blocked; never proceed as if the user had chosen.
